...
const Dashboard: React.FC = () => {
  ...
  }) => <TableHead className="cursor-pointer hover:bg-accent/50 transition-colors duration-200 select-none" onClick={() => handleSort(sortKeyName)}>
      <div className="flex items-center gap-2 font-semibold">
        <Icon className="h-4 w-4 text-primary" />
        <span>{label}</span>
        <ArrowUpDown className={cn("h-3 w-3 transition-colors", sortKey === sortKeyName ? "text-primary" : "text-muted-foreground")} />
      </div>
    </TableHead>;
  ...
  const bankChartData = useMemo(() => {
    ...
    return Object.entries(bankCounts).map(([name, count]) => ({
      name,
      count
  ...
  return <div className="animate-fade-in space-y-6">
              ...
              bottom: 40
                ...
                fontWeight: 600
                  ...
                  <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                    {bankChartData.map((_, index) => <Cell key={`cell-${index}`} fill={chartColors[index % chartColors.length]} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
      ...
      <Card className="shadow-lg border-0 overflow-hidden">
        ...
        <CardContent className="p-6">
          ...
          {invoices.length === 0 ? <div className="text-center py-16">
            ...
            </div> : <div className="overflow-x-auto rounded-xl border border-border shadow-sm" ref={printRef}>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {sortedInvoices.map((inv, index) => <TableRow key={inv.id} className={cn("transition-all duration-200 hover:bg-muted/50", index % 2 === 0 ? "bg-card" : "bg-muted/10")}>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          ...
                          <Button variant="ghost" size="icon" className="h-9 w-9 hover:bg-destructive/10 text-destructive transition-colors" onClick={() => handleDeleteSingle(inv.id)}>
      ...
      <EditInvoiceDialog invoice={editingInvoice} open={!!editingInvoice} onOpenChange={open => !open && setEditingInvoice(null)} />
